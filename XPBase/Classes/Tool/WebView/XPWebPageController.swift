...
/// Web页面控制器（XP命名空间版本），用于加载和显示网页
/// 支持进度条显示、JavaScript交互、导航栏自动适配等功能
///
/// 使用示例：
/// ```swift
/// // 创建并配置 XPWebPageController
/// let webVC = XPWebPageController()
...
/// ```
public class XPWebPageController: UIViewController {
    // MARK: - 公开属性
    /// 网页URL字符串
    ...
    /// 导航栏是否透明
    public var navBackEmpty: Bool = false
    /// 是否调整顶部内容缩进
    public var isAdjustTop: Bool = false
    /// 控制器样式
    public var style: XPWebPageControllerStyle = .normal
    /// JavaScript交互名称
    public let webUserContentControllerName = "sendSuccess"
    /// JavaScript交互名称列表
...
extension XPWebPageController {
    ...
    private func updateNavigationTitle() {
        if navTitle.isEmpty, let title = webView.title, !title.isEmpty {
            gk_navTitle = title
        }
    }
    ...
    private func handleGoodsCategoryAction(categoryPath: String) {
        ...
        // 示例代码：
        // XPNavigator.share.open(StoresUrlRouter.storesList.rawValue + "?path=\(categoryPath)&style=specific")
    }
...
extension XPWebPageController: WKNavigationDelegate {
    // 页面开始加载时调用
